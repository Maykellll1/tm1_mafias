...
AddEventHandler('tm1_mafias:getStockItem', function(type, item, count, society)
	local _source      = source
	local xPlayer      = ESX.GetPlayerFromId(_source)
		...
		TriggerEvent('esx_addoninventory:getSharedInventory', society, function(inventory)
			...
			if count > 0 and inventoryItem.count >= count then
				...
				if sourceItem.limit ~= -1 and (sourceItem.count + count) > sourceItem.limit then
					TriggerClientEvent('esx:showNotification', _source, 'No puedes cogerlo')
				else
				...
				TriggerClientEvent('esx:showNotification', _source, 'No tienes suficiente cantidad en la mafia')
			end
		end)
...
AddEventHandler('tm1_mafias:putStockItems', function(type, item, count, society)
	local _source      = source
	local xPlayer      = ESX.GetPlayerFromId(_source)
	...
	if type == 'item_standard' then
		...
		if playerItemCount >= count and count > 0 then
			TriggerEvent('esx_addoninventory:getSharedInventory', society, function(inventory)
				...
				TriggerClientEvent('esx:showNotification', _source, 'Has depositado '..inventory.getItem(item).label)
			end)
		else
...
ESX.RegisterServerCallback('tm1_mafias:getVehicles', function(source, cb)
	local _source = source
	local xPlayer = ESX.GetPlayerFromId(_source)
	local vehicules = {}
	MySQL.Async.fetchAll("SELECT * FROM owned_vehicles WHERE owner=@identifier",{['@identifier'] = xPlayer.getIdentifier()}, function(data) 
		for _,v in pairs(data) do
			...
			table.insert(vehicules, {vehicle = vehicle, state = v.stored, id = v.id, type = v.type, job = v.job})
		end
		cb(vehicules)
	end)
end)

ESX.RegisterServerCallback('tm1_mafias:checkMoney', function(source, cb)

	local xPlayer = ESX.GetPlayerFromId(source)

	if xPlayer.get('money') >= 44 then
...
ESX.RegisterServerCallback('tm1_mafias:getOutVehicles',function(source, cb)	
	local _source = source
	local xPlayer = ESX.GetPlayerFromId(_source)
	local vehicules = {}

	MySQL.Async.fetchAll("SELECT * FROM owned_vehicles WHERE owner=@identifier AND stored=false",{['@identifier'] = xPlayer.getIdentifier()}, function(data) 
		for _,v in pairs(data) do
			local vehicle = json.decode(v.vehicle)
			vehicle.job = v.job
			vehicle.type = v.type
...
RegisterServerEvent('tm1_mafias:modifystate')
AddEventHandler('tm1_mafias:modifystate', function(state,plate)
	local _source = source
	local xPlayer = ESX.GetPlayerFromId(_source)
...
RegisterServerEvent('tm1_mafias:handcuff')
AddEventHandler('tm1_mafias:handcuff', function(target)
	local xPlayer = ESX.GetPlayerFromId(source)
	TriggerClientEvent('tm1_mafias:handcuff', target)
end)

RegisterServerEvent('tm1_mafias:drag')
AddEventHandler('tm1_mafias:drag', function(target)
	local xPlayer = ESX.GetPlayerFromId(source)
	TriggerClientEvent('tm1_mafias:drag', target, source)
end)

RegisterServerEvent('tm1_mafias:putInVehicle')
AddEventHandler('tm1_mafias:putInVehicle', function(target)
	local xPlayer = ESX.GetPlayerFromId(source)
	TriggerClientEvent('tm1_mafias:putInVehicle', target)
end)

RegisterServerEvent('tm1_mafias:OutVehicle')
AddEventHandler('tm1_mafias:OutVehicle', function(target)
	local xPlayer = ESX.GetPlayerFromId(source)
	TriggerClientEvent('tm1_mafias:OutVehicle', target)
end)
